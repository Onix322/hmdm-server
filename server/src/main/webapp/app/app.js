angular
            ...
            goNewTab: function (stateName, params) {
              $window.open(
                document.location.origin +
                  ...
                  $delegate.href(stateName, params, { absolute: false }),
                "_blank",
              );
            },
          };
  ...
  .constant("SUPPORTED_LIBS", [
    {
      name: "leaflet",
      files: ["ext/leaflet-1.5.1/leaflet.js"],
      styles: ["ext/leaflet-1.5.1/leaflet.css"],
    },
    {
      name: "leaflet.markercluster",
      files: ["ext/leaflet.markercluster-1.4.1/leaflet.markercluster.js"],
      styles: ["ext/leaflet.markercluster-1.4.1/MarkerCluster.css"],
    },
    {
  ...
  .config([
    "$ocLazyLoadProvider",
    "SUPPORTED_LIBS",
    function ($ocLazyLoadProvider, SUPPORTED_LIBS) {
  ...
  .factory(
    "externalLibLoader",
    function ($rootScope, $ocLazyLoad, $timeout, $css, SUPPORTED_LIBS) {
      var libs = {};
      SUPPORTED_LIBS.forEach(function (lib) {
        libs[lib.name] = angular.extend(angular.copy(lib, {}), {
          loadedFiles: [],
          loading: false,
          loaded: false,
        });
          ...
          if (!library.loaded) {
            if (!library.loading) {
              ...
              loader = function () {
                ...
                return new Promise(function (resolve) {
                  var listenerRemove = $rootScope.$on(
                    ...
                    function (e, url) {
                      if (library.files.indexOf(url) >= 0) {
                        ...
                        if (
                          library.files.length === library.loadedFiles.length
                        ) {
                          library.loaded = true;
  ...
  .run(
    function (
      ...
      $window.document.title = localization
        .localize("app.title")
        .replace("${appName}", "MDM");
      rebranding.query(function (value) {
        $window.document.title = localization
          .localize("app.title")
          .replace("${appName}", value.appName);
      });

      $rootScope.$on(
        "aero_LANGUAGE_SETTINGS_UPDATED",
      ...
      $transitions.onStart({}, function (trans) {
        ...
        if (
          trans.to().name !== "passwordRecovery" &&
